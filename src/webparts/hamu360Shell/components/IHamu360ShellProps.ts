 ...
 * though it now mounts `ThemeProvider` and the Sprint 5 dashboard showcase
 * (see `DashboardWidgetShowcase`) instead of a static placeholder.
 *
 ...
 * `quickLinks` are Sprint 3's three Hero datasets. `events`/`newJoiners`/
 * `regulatoryUpdates`/`firmWins` are Sprint 5's four Dashboard Widget
 * datasets. All eight are resolved once in `Hamu360ShellWebPart.onInit()`
 * via the matching `ServiceFactory.create*Service()` calls and handed down
 * as plain data from here on; nothing below this file talks to a service
 * directly.
 */
