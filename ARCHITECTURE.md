...
# Sprint 7 — Enterprise UI Polish & Wireframe Fidelity

**A documentation note before this section starts:** this file's own table of contents jumps from Sprint 1 straight to Sprint 7. Sprints 2–6 shipped real, substantial architecture (the application shell, hero section, hub cards, dashboard widgets, and the full SharePoint/Graph integration) but never appended a section here — each sprint's reasoning instead lives in its own component docblocks, `DESIGN_TOKEN_DEBT.md`, `ARCHITECTURE_DEBT.md`, and the Sprint 6 checklist/hard-gate documents at the repo root. That gap predates this sprint and is outside a presentation-only polish sprint's scope to backfill; flagged here rather than silently leaving the reader to wonder why the numbering skips five sprints of real work.

Sprint 7's brief was explicit: no new features, no service/data/routing changes — a pure presentation pass, closing the gap between the shipped product and the approved wireframe, plus paying down whatever `DESIGN_TOKEN_DEBT.md` debt is resolvable without an architecture change. Every file touched this sprint is a `.module.scss`, a `.tsx` template (never a service, hook, or type contract), or documentation.

## 27. Visual refinement decisions

**Design token debt resolved (`DESIGN_TOKEN_DEBT.md` entries 2 and 4):**

- **`accentBackground`/`accentForeground`** added to `ISemanticColorTokens` (`src/theme/colors.ts`) — the wireframe's pale-gold/dark-gold (`--gl`/`--gd`) pair, reproduced exactly in light mode from palette values that already existed but were never exposed. All four sites that previously faked this pairing with a `currentColor` + opacity pseudo-element (`HubCard.iconAccentTint`, `EventsWidget.dateBox`, `NewJoinersWidget.avatar`, `FirmWinsWidget.iconBox`/`.tag`) now use the real, flat token pair — removing an extra DOM pseudo-element at each site.
- **`GridColumns` widened to include `5`** (`src/components/Grid/IGridProps.ts`), with a matching `.columns5` ramp in `Grid.module.scss` copied verbatim from `HubCardGrid`'s former hand-rolled local grid. `HubCardGrid.tsx` now renders through the shared `Grid` primitive (`as="ul" columns={5}`); `HubCardGrid.module.scss`, which held nothing but the now-redundant local ramp, was deleted.
- Entries 1, 3, and 5 were evaluated and deliberately left open, each with a concrete, sharpened recommendation for whoever picks them up next — see `DESIGN_TOKEN_DEBT.md` directly rather than duplicating that reasoning here. In short: entry 1 needs a design decision (how many overlay-opacity steps is too many) that a polish sprint shouldn't make unilaterally; entries 3 and 5 need a visual-verification loop (a real browser, screenshots against the wireframe) this sandboxed build environment cannot provide, so batch-migrating ~10 call sites blind was judged too risky to do quietly.

**A real visual regression found and fixed:** `HomePage.module.scss`'s `.sectionHeading` class (used by the "Team hubs" and "What's happening at Hamu Legal" section headers) had no typography rule at all — it was silently inheriting `body`'s 13px/400 style via `global.scss`'s `h1..h6 { font-size: inherit; font-weight: inherit; }` fallback, meaning both section headers rendered visually identical to body copy with zero heading hierarchy. This was never a documented compromise — it was an oversight, caught by this sprint's "identify every visual difference before changing anything" audit. Fixed to match the wireframe's `.slbl` (9px/700, uppercase, muted color) via the nearest existing role (`label`, 10px/600) plus `text-transform`/`color` as genuine additions, not mixin-property overrides.

**Decorative hero background restored:** the wireframe's two soft radial-gradient circles behind the hero band (`.hero::before`/`::after`, a white blob and a gold blob) had been omitted entirely in Sprint 3 for lacking a token match. Reproduced this sprint using existing tokens (`$color-text-on-primary`/`$color-accent` at `token-opacity(hover-overlay)`, the nearest existing step to the wireframe's exact 0.04/0.06 values) rather than new raw rgba literals, with `z-index: -1` to sit behind in-flow content correctly.

**Interactive card motion added:** `HubCard` (the only genuinely interactive, button-based card in the app) gained a hover/focus-visible lift (`translateY(-2px)` + `shadow(elevation2)`), wrapped in `@include motion-safe`, using the existing `motion(scale)` (180ms) preset — no new duration/easing tokens invented. The four dashboard-widget cards and the three hero cards deliberately do **not** get this treatment: they are static content, not controls, and adding hover elevation to a non-interactive element would visually promise an interaction that doesn't exist — this also matches the wireframe's own restraint (its one observed `box-shadow` sits on the whole page frame, never an individual card).

**A missing interactive state fixed:** `QuickLinksGrid`'s `.tile` (a real `<button>`) had no hover or focus-visible feedback at all — the one interactive element in the app with zero state change on interaction. Fixed using the exact technique `NavigationItem.active:hover` already established (deepen the existing wash from `active-overlay` to `full` opacity), not a new pattern.

## 28. Accessibility improvements — and one unresolved finding

Global focus handling (`:focus-visible` + `focus-ring` mixin), reduced-motion enforcement (a blanket `@media (prefers-reduced-motion: reduce)` rule in `global.scss`), and semantic interactive elements (`HubCard`/`QuickLinksGrid`'s tiles are real `<button>`s, not styled `<div>`s) were already in place from earlier sprints and needed no rework — this sprint's audit confirmed rather than rebuilt them. `HubCard`'s new hover lift applies identically on `:focus-visible`, so keyboard users get the same affordance mouse users do, not a lesser one.

**Unresolved, flagged rather than silently shipped: a contrast failure in the wireframe's own gold pairing.** Computing the actual WCAG contrast ratio for `accentForeground` (`#b8861a`) on `accentBackground` (`#f5e4b0`) — the pair just added to close `DESIGN_TOKEN_DEBT.md` entry 2 — gives **2.57:1**, well under the 4.5:1 AA threshold for normal text (and under 3:1 even by the more lenient large-text/non-text threshold). This pairing is used for real text in three places (`EventsWidget`'s date-box day/month numerals, `NewJoinersWidget`'s avatar initials, `FirmWinsWidget`'s category tag text), not just icon glyphs. This is not a bug introduced by the token addition — `#b8861a`/`#f5e4b0` are the wireframe's own `--gd`/`--gl` values, reproduced exactly per this design system's "colors are reproduced verbatim, never approximated" principle (§14) — but faithful reproduction and WCAG AA are in direct tension here, and resolving that tension by quietly darkening a color would itself violate the "reproduce the wireframe exactly" principle without anyone deciding to make that trade-off. **Left exactly as the wireframe specifies, not silently patched**, with a concrete fix computed and ready: darkening `accentForeground` by roughly 30% (`#805d12` computed via linear RGB scaling) reaches 4.76:1, comfortably passing, while staying visually in the same dark-gold family as the original. This needs an explicit decision from whoever owns visual sign-off — accept the documented gap, or trade a small, deliberate deviation from pixel-exact wireframe color for AA compliance — not a unilateral change in a presentation-only sprint.

## 29. Responsive improvements

No breakpoint-behavior defects were found. `Grid`'s mobile-first column ramps (now including the new `columns5` step), `Container`'s gutter widening, and `Section`'s vertical rhythm were already systematically breakpoint-driven from Sprint 1 (`from-tablet`/`from-laptop`/`from-desktop` mixins, `px`-based thresholds — see §16), and every component built in Sprints 2–6 was already required to build on top of them rather than hand-rolling responsive behavior. The one hand-rolled exception (`HubCardGrid`'s local grid) is now migrated to the shared primitive (§27), which was the last outstanding case of a component bypassing the standard responsive system.

## 30. Performance improvements

No `React.memo`/`useMemo`/`useCallback` additions were made. This app fetches every dataset once in `Hamu360ShellWebPart.onInit()` and re-renders on exactly two triggers: that fetch resolving, and a SharePoint tenant theme change (`onThemeChanged`) — there is no list virtualization need, no high-frequency state update, and no measured re-render cost anywhere in the component tree to justify memoization. Adding it here would be exactly the "premature optimization" this sprint's brief explicitly warned against. A pass for unused imports, duplicate CSS, and stray `console.*`/`debugger` statements found none beyond what `eslint`/`prettier` already catch automatically on every build (see §31).

## 31. Verification

`npm run lint`, `npm run type-check`, and `npm run format:check` all pass with zero errors on every file this sprint touched. See the Sprint 7 closeout summary (delivered alongside this update, not duplicated here) for the full verification transcript, including the `gulp build --ship`/`bundle --ship`/`package-solution --ship` results and this sandbox's known limitations running SPFx's production packaging pipeline.

## 32. Remaining UI debt

Tracked in full in `DESIGN_TOKEN_DEBT.md` (entries 1, 3, 5 — all open, all with a specific recommended next step) and `ARCHITECTURE_DEBT.md` (the `AsyncState<T,E>` per-widget loading/error granularity gap, unchanged by this sprint — Sprint 7's brief explicitly forbade touching data contracts, so this remains exactly where Sprint 6 left it). The one new item this sprint adds to the register is §28's contrast finding — recorded here and in the Sprint 7 closeout summary, not yet in a dedicated debt file entry since it's a single, already-fully-specified finding rather than a recurring pattern.

## 33. Sprint A5 — the token bridge was silently fallible, now it isn't

**The structural issue (found, not worked around):** the SCSS half of the design-token bridge (`src/styles/_tokens.scss`) is _name-based_ — `shadow(elevation-2)` compiles to `var(--h360-shadow-elevation-2)` purely by string interpolation. Nothing ever checked that the key exists, so a typo'd key compiled cleanly into a CSS variable no `ThemeProvider` ever defines, and the declaration silently computed to nothing at runtime. This was not hypothetical: Sprint 7 shipped `HubCard`'s hover "lift + elevation" with `shadow(elevation2)` (missing hyphen) — the elevation half of that flagship micro-interaction **never rendered**, and no lint, type-check, or build step could have caught it.

**The fix (root cause, not the symptom):** `space()`, `radius()`, `shadow()`, and `token-opacity()` now validate their key against the canonical `$*-keys` lists and fail the build with `@error` on an unknown key. The whole bug class is now a compile-time failure. (The `typography()` mixin remains unguarded — its role list is consumed via a documented `$typography-roles` list too, but a wrong role fails visibly at a glance since _five_ properties go missing at once; extend the same guard there if it ever bites.)

**Also formalized this sprint:** `AppShell` now owns the wireframe's `.hos` page-frame treatment (surface, hairline border, radius, the `shadows.hover` value that had been kept verbatim-but-unused since Sprint 1). One structural constraint documented in the code: the wireframe achieves its corner rounding with `overflow: hidden` on the frame, but an overflow-clipping ancestor disables `position: sticky` descendants — which would have silently killed the sticky `Header`. The frame therefore rounds via matched per-corner radii on `Header`/`Footer` instead. If a future sprint ever adds an ancestor `overflow: hidden`/`clip` around `AppShell`, the sticky header is the first thing to re-test.
